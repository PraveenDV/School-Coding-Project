...
def write_head():
    with open('global_announcements.csv', 'w', newline='') as file1:
                writer = csv.writer(file1)
                text='Global Announcements'
                writer.writerow(text)
    with open('admin_announcements.csv', 'w', newline='') as file2:
                writer = csv.writer(file2)
                text='Admin Announcements'
                writer.writerow(text)
write_head()

glb_df=pd.read_csv('global_announcements.csv')
admin_df=pd.read_csv('admin_announcements.csv')
...
def open_announcement_editor(is_global):
    ...
    def save_announcement(announcement_text):
        if is_global:
           # global_announcements.append(announcement_text)
            ...
            glb_df['Announcements']=announcement_text
            with open('global_announcements.csv', 'a', newline='') as file1:
                writer = csv.writer(file1)
                writer.writerow([announcement for announcement in glb_df['Announcements']])
        else:
            #admin_announcements.append(announcement_text)
            #print(admin_announcements)        
            admin_df['Announcements']=announcement_text
            with open('admin_announcements.csv', 'a', newline='') as file2:
                writer = csv.writer(file2)
                writer.writerow([announcement for announcement in admin_df['Announcements']])
    ...
    send_on_app_button = ctk.CTkButton(announcement_editor_window, 
...
glb_df.dropna(axis=0)
admin_df.dropna(axis=0)
