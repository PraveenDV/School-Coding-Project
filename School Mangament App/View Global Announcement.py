...
def open_global_announcements_viewer():
    global_announcements_viewer = ctk.CTk()
    ...
    for announcement in announcements:
        ...
        announcement_label = ctk.CTkLabel(global_announcements_viewer, text=announcement_text, wraplength=500, justify="left")
        announcement_label.pack(pady=5, padx=10)
        if announcement[2]:
            download_button = ctk.CTkButton(global_announcements_viewer, text="Download Attachment", command=lambda path=announcement[2]: download_attachment(path))
